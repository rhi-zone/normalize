...
- `worktrees/` — temporary agent worktrees created during isolated task execution (excluded from walkers via `[walk] exclude = [".claude/worktrees/"]`; not tracked by git). Each is a real git worktree sharing the repo's object database; isolated agents edit only their own worktree copy.
