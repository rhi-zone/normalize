//! Git history hotspot analysis — typed compute.

use crate::complexity::max_function_complexity;
...
#[cfg(feature = "cli")]
mod present {
    use super::{FileHotspot, HotspotsReport};
    ...
    fn pretty_hotspots_data(
        ...
        let title = hotspots_title(hotspots, recency_weighted);
        if has_complexity {
            pretty_ranked_table(&title, hotspots, None, |_| None)
        } else {
            let entries: Vec<FileHotspotNoComplexity<'_>> =
                hotspots.iter().map(FileHotspotNoComplexity).collect();
            pretty_ranked_table(&title, &entries, None, |_| None)
        }
    }
            ...
            if let Some(ref repos) = self.repos {
                ...
                for entry in repos {
                    ...
                    if let Some(ref err) = entry.error {
                        ...
                        parts
                            .push("No hotspots found (no git history or source files)".to_string());
                    } else {
...
/// Compute max cyclomatic complexity per file (max over per-function complexities).
fn compute_file_complexities(root: &Path, paths: &[String]) -> HashMap<String, usize> {
    paths
        ...
        .filter_map(|path| {
            ...
            let max_complexity = max_function_complexity(&full_path, &content)?;
