...
pub(crate) fn parse(argv: impl Iterator<Item = std::ffi::OsString>) -> ParseResult<HiArgs> {
    parse_low(argv).and_then(|low| match HiArgs::from_low_args(low) {
...
fn parse_low(argv: impl Iterator<Item = std::ffi::OsString>) -> ParseResult<LowArgs> {
    // Collect argv so we can iterate over it twice (first parse + config re-parse).
    let argv: Vec<std::ffi::OsString> = argv.collect();

    if let Err(err) = crate::rg::logger::Logger::init() {
    ...
    if let Err(err) = parser.parse(argv.iter().cloned(), &mut low) {
        return ParseResult::Err(err);
    }
    ...
    // to re-parse.
    let config_args = crate::rg::flags::config::args();
    ...
    final_args.extend(argv.iter().cloned());
...
fn set_log_levels(low: &LowArgs) {
    crate::rg::messages::set_messages(!low.no_messages);
    crate::rg::messages::set_ignore_messages(!low.no_ignore_messages);
    match low.logging {
