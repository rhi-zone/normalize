...
# `const MAX_RETRIES = 3` makes the code self-documenting and the value
# easy to change consistently.
#
# ## How to fix
#
...
; Matches number literals in binary comparison expressions, excluding
; single-digit numbers (0-9) which are usually obvious from context.
((binary_expression
  operator: [">" "<" ">=" "<=" "==" "!=" "===" "!=="]
  right: (number) @_num) @match
 (#not-match? @_num "^[0-9]$"))

; Also match when the number is on the left side (yoda conditions)
((binary_expression
  left: (number) @_num
  operator: [">" "<" ">=" "<=" "==" "!=" "===" "!=="]) @match
 (#not-match? @_num "^[0-9]$"))
