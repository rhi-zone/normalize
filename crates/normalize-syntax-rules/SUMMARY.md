...
Includes 54 builtin rules (50 syntax) across Rust, JavaScript/TypeScript, Python, Go, and Ruby (debug prints, security patterns, style issues, idiomatic patterns), plus cross-language rules (`no-todo-comment`, `no-fixme-comment`, `hardcoded-secret`). Recent additions: `ruby/double-negation`, `ruby/open-struct`, `python/use-enumerate`, `python/raise-without-from`, `typescript/no-empty-interface`, `typescript/no-inferrable-types`. The `SourceRegistry` / `RuleSource` trait system provides namespace-keyed data (path, env, git, language-specific) used to evaluate `requires` predicates. Fixture tests in `tests/` support `match.*`, `no_match.*`, and `fix.*`/`fix.expected.*` files for testing detection and auto-fix transforms.
