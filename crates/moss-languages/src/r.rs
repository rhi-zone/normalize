...
impl Language for R {
    ...
    fn extensions(&self) -> &'static [&'static str] { &["r", "R", "rmd", "Rmd"] }
    fn grammar_name(&self) -> &'static str { "r" }
    ...
    fn extract_function(&self, node: &Node, content: &str, _in_container: bool) -> Option<Symbol> {
        ...
        let name = parent.child(0).map(|n| content[n.byte_range()].to_string())?;
    ...
    fn is_public(&self, node: &Node, content: &str) -> bool {
        node.child(0).map_or(true, |n| !content[n.byte_range()].starts_with('.'))
    }
    ...
    fn lang_key(&self) -> &'static str { "r" }

    fn is_stdlib_import(&self, import_name: &str, _project_root: &Path) -> bool {
        matches!(import_name, "base" | "stats" | "graphics" | "grDevices" |
            "utils" | "datasets" | "methods" | "grid" | "tools" | "compiler")
    }
    ...
    fn indexable_extensions(&self) -> &'static [&'static str] { &["r", "R"] }
    ...
    fn should_skip_package_entry(&self, name: &str, is_dir: bool) -> bool {
        ...
        if is_dir && (name == "man" || name == "inst") { return true; }
