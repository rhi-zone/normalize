...
- `store.rs` — filesystem I/O: unit CRUD, jq transform/predicate eval via jaq, BFS walk_from, legacy `edges.jsonl` migration (all behind `cli` feature for jq functions); jq representation uses `metadata.links` (links embedded in metadata) to match user-facing API
