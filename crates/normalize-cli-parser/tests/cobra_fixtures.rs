...
fn test_parse_main_help() {
    let spec = parse_help_with_format(EXAMPLE_HELP, "cobra").expect("should parse");
    ...
    // Check commands (help/completion filtered out)
    ...
    assert_eq!(config.unwrap().value, Some("<string>".to_string()));
