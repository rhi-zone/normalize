...
For the `normalize docs` command the crate also carries symbol-documentation infrastructure: `SymbolDoc`/`DocFormat` types, Rust extractors (`local_docs`, `docs_rs`), and shared multi-language primitives — `source_archive` (download + extract package source archives to the cache) and `doc_tree` (tree-sitter-based docstring/signature extraction from an extracted source tree). These depend on `normalize-languages` (the `Language` trait + parsers) and `normalize-local-deps`.
