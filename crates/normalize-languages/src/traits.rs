...
// === Refactor codegen capability ===

/// A parameter of a function being generated by the refactoring engine.
///
/// Plain owned data — the `normalize-refactor` recipe maps its own `Parameter`
/// type into this at the call boundary (refactor depends on languages, never the
/// reverse).
...
pub struct GenParam {
    ...
    /// Whether the binding in the original function is `mut` (Rust).
    pub mutable: bool,
}

/// The return shape of a function being generated by the refactoring engine.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum GenReturn {
    /// No return value.
    Unit,
    /// A single named return value.
    Single(String),
    /// Multiple named return values (tuple / multi-return).
    Tuple(Vec<String>),
    /// A fallible return (`Result<ok, err>` in Rust, `(ok, error)` in Go, etc.)
    /// produced when exception edges escape the extraction boundary.
    Result(String, String),
}

/// Specification for generating an extracted function definition.
#[derive(Debug, Clone)]
pub struct ExtractedFnSpec {
    /// Name of the new function.
    pub name: String,
    /// Parameters inferred as live-in to the region.
    pub params: Vec<GenParam>,
    /// Return shape inferred from live-out variables.
    pub ret: GenReturn,
    /// Whether the region contained `await` / async semantics.
    pub is_async: bool,
    /// Whether the region contained generator (`yield`) semantics.
    pub is_generator: bool,
    /// The body lines (already common-indent-stripped).
    pub body_lines: Vec<String>,
    /// The indentation prefix to apply to the generated definition.
    pub indent: String,
}

/// Specification for generating the call site that replaces an extracted region.
#[derive(Debug, Clone)]
pub struct CallSiteSpec {
    /// Name of the function being called.
    pub name: String,
    /// Parameters (their names become positional arguments).
    pub params: Vec<GenParam>,
    /// Return shape (drives how the result is bound at the call site).
    pub ret: GenReturn,
    /// Whether the call is awaited.
    pub is_async: bool,
    /// The indentation prefix to apply to the generated call site.
    pub indent: String,
}

/// Capability trait: language can generate source for the refactoring engine.
///
/// Implementations own the language-specific judgment that genuinely belongs in
/// Rust (per-language code synthesis), as opposed to node-kind classification
/// (which lives in `*.refactor.scm` queries). Access via
/// `lang.as_refactor_codegen()`.
///
/// Note: structural classification (which node is a function/call/scope) does NOT
/// belong here — that is `.scm` query work. This trait is strictly about emitting
/// new source text.
pub trait RefactorCodeGen: Language {
    /// Format a single parameter for a function signature (add-parameter recipe).
    fn format_param(&self, name: &str, ty: Option<&str>) -> String;

    /// Render a variable binding statement (introduce-variable recipe).
    /// e.g. `let name = expr;\n`, `const name = expr;\n`, `name = expr\n`.
    fn render_binding(&self, name: &str, expr: &str, indent: &str) -> String;

    /// Render a complete extracted function definition.
    fn render_function(&self, spec: &ExtractedFnSpec) -> String;

    /// Render the call site that replaces an extracted region.
    fn render_call_site(&self, spec: &CallSiteSpec) -> String;

    /// Whether the language can return multiple values directly (e.g. Go, Python
    /// tuples, TS arrays). Default `false`.
    fn supports_multi_return(&self) -> bool {
        false
    }

    /// Whether the language models fallible returns via a `Result`-like type that
    /// the recipe should propagate (Rust). Default `false`.
...
pub trait Language: Send + Sync {
    ...
    /// Capability query: returns `Some(self)` if this language can generate source
    /// for the refactoring engine (extract-function bodies, call sites, variable
    /// bindings, parameter formatting). Returns `None` for languages whose codegen
    /// has not been implemented yet.
    /// Implement `RefactorCodeGen` and override this to opt in.
    fn as_refactor_codegen(&self) -> Option<&dyn RefactorCodeGen> {
        None
    }
