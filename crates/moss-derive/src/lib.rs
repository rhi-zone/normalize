...
pub fn derive_merge(input: TokenStream) -> TokenStream {
        ...
        Data::Struct(data) => match &data.fields {
            Fields::Named(fields) => {
                let field_merges = fields.named.iter().map(|f| {
                    ...
                    quote! {
                        #field_name: crate::merge::Merge::merge(self.#field_name, other.#field_name)
                    }
                });
                quote! {
                    Self {
            ...
            Fields::Unnamed(fields) => {
                let field_merges = (0..fields.unnamed.len()).map(|i| {
                    ...
                    quote! {
                        crate::merge::Merge::merge(self.#index, other.#index)
                    }
                });
                quote! {
                    Self(#(#field_merges),*)
    ...
    let expanded = quote! {
        impl #impl_generics crate::merge::Merge for #name #ty_generics #where_clause {
