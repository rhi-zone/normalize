...
//!
//! Supports external index locations via `NORMALIZE_INDEX_DIR`. This allows repos
//! without `.normalize` in `.gitignore` to store indexes elsewhere. Lives in
//! `normalize-facts` (the lowest crate that resolves this path — the `structure`
//! service and `FileIndex` storage both need it) so `normalize-index` and the main
//! crate can share one primitive. `normalize-index` re-exports it.
