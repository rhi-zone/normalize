//! Snapshot tests for the Go CFG builder and Mermaid renderer.
//!
//! Tests are skipped gracefully if the Go grammar is not installed.

use normalize_cfg::{FunctionId, builder::build, mermaid::render};
use normalize_languages::parsers::grammar_loader;
use streaming_iterator::StreamingIterator;

fn build_cfg_mermaid_go(fixture_path: &str, function_name: Option<&str>) -> String {
    let source = std::fs::read(fixture_path)
        .unwrap_or_else(|e| panic!("failed to read {fixture_path}: {e}"));

    let loader = grammar_loader();
    let ts_lang = match loader.get("go") {
        Ok(l) => l,
        Err(_) => return "go grammar not installed — skipped".to_string(),
    };
    let cfg_query = match loader.get_cfg("go") {
        Some(q) => q,
        None => return "no go cfg query — skipped".to_string(),
    };
    ...
    let tags_query_src = match loader.get_tags("go") {
        Some(q) => q,
        None => return "no go tags query — skipped".to_string(),
    };
...
#[test]
fn test_go_linear() {
    let mermaid = build_cfg_mermaid_go("tests/fixtures/go/linear.go", Some("linear"));
    insta::assert_snapshot!(mermaid);
}

#[test]
fn test_go_branch() {
    let mermaid = build_cfg_mermaid_go("tests/fixtures/go/branch.go", Some("branch"));
    insta::assert_snapshot!(mermaid);
}

#[test]
fn test_go_loop() {
    let mermaid = build_cfg_mermaid_go("tests/fixtures/go/loop_.go", Some("loop_"));
    insta::assert_snapshot!(mermaid);
}

#[test]
fn test_go_early_return() {
    let mermaid = build_cfg_mermaid_go("tests/fixtures/go/early_return.go", Some("earlyReturn"));
    insta::assert_snapshot!(mermaid);
}
