...
- `lib.rs` — data model: `Cfg`, `BasicBlock`, `Edge`, `BlockId`, `FunctionId`, `BlockKind`, `EdgeKind`; re-exports `builder`, `mermaid`, `service`
- `builder.rs` — structured-CFG builder; takes a tree-sitter `Tree`, a `.cfg.scm` query string, and a body byte range; walks CST nodes classified by capture names to build the CFG graph
