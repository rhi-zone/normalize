...
- `git_ops.rs` — thin re-export wrapper over `normalize-git`: re-exports `diff_base_to_head`, `read_blob_text`, `read_blob_bytes`, `walk_tree_at_ref`, `FileChangeKind`, `FileChange`; provides a local `open_repo` adapter that returns `anyhow::Result` (budget callers propagate errors via `?`)
