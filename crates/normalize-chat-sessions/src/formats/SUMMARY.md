...
- `mod.rs` — `LogFormat` trait (`name`, `sessions_dir`, `projects_root` (optional, default `None`; used by `--all-projects` listings), `metadata_roots`, `list_sessions`, `detect`, `parse`); `ParseError` enum (`Io`, `Format`, `Other`) for typed parse errors replacing `String`; global format registry (`FORMATS`, `register`, `get_format`, `detect_format`, `list_formats`); `project_metadata_roots(project)` free function returns all existing metadata dirs across all formats (used by `normalize sync`); `FormatRegistry` for isolated use; `parse_session`/`parse_session_with_format` returning `Result<Session, ParseError>`; helpers `list_jsonl_sessions`, `peek_lines`, `read_file`
