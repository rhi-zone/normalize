...
- `git_staleness.rs` — git-based staleness score for symbol embeddings: counts repo commits elapsed since a file was last touched; used in ANN re-ranking (`score = cosine_sim * (1 - 0.3 * staleness)`)
- `schema.rs` — SQLite DDL for the `embeddings` table (migrates-in alongside existing schema); also registers `sqlite-vec` (`vec0` virtual table) via `vec_ext.rs`
- `vec_ext.rs` — sqlite-vec extension registration for approximate nearest-neighbor search
- `store.rs` — read/write embeddings to/from SQLite (upsert, bulk-load, delete-by-path)
- `search.rs` — brute-force ANN search + staleness re-ranking
