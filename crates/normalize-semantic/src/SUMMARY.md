...
- `populate.rs` — `populate_embeddings()`: walks the structural index, builds chunks, batches through the embedder, writes to store; opens its own `VecConnection` for ANN table operations; full rebuild drops+recreates tables and VACUUMs; incremental uses `INSERT OR REPLACE`; emits progress to stderr
