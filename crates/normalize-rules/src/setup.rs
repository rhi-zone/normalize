...
//! through each rule that has violations — prompting to enable/disable.
...
pub fn run_setup_wizard(root: &Path) -> i32 {
    ...
    // Build map: rule_id -> (description, severity, enabled, type, recommended)
    ...
    // Sort: recommended first, then by violation count desc, then alphabetically
    let mut rules_with_violations: Vec<(String, Vec<normalize_output::diagnostics::Issue>)> =
        by_rule.into_iter().collect();
    rules_with_violations.sort_by(|a, b| {
    ...
    // Count rules with zero violations for the summary
    let rules_with_zero = rule_meta.len() - rules_with_violations.len();
