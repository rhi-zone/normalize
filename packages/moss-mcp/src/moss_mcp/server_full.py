...
from moss_mcp.server import _check_mcp, _should_use_ephemeral
...
def create_server() -> Any:
    ...
    async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent | ResourceLink]:
        ...
        """
        from moss_orchestration.cache import get_ephemeral_cache
        ...
        try:
            ...
            from moss_orchestration.errors import handle_error
...
def _get_tool_dispatcher() -> dict[str, Any]:
    """Get mapping from api_path to handler functions.

    Returns a dict mapping "subapi.method" to callable functions.
    The api_paths must match those generated by introspect_api().
    """
    from moss_intelligence.anchors import Anchor, find_anchors, resolve_anchor
    ...
    from moss_intelligence.tree import build_tree, get_all_files, render_tree
    from moss_orchestration.dwim import analyze_intent, list_tools, resolve_tool

    def _read_file(file_path: str | Path | None) -> str:
        if not file_path:
            return ""
        p = Path(file_path)
        return p.read_text() if p.exists() else ""

    # Map api_path to handler function
    # api_paths are: "{subapi}.{function_name}" from introspect_api()
    return {
        # skeleton module
        "skeleton.extract_python_skeleton": lambda root, source=None, file_path=None, **kw: (
            extract_python_skeleton(source if source else _read_file(file_path))
        ),
        "skeleton.format_skeleton": lambda root, skeleton=None, file_path=None, **kw: (
            format_skeleton(
                skeleton if skeleton else extract_python_skeleton(_read_file(file_path)),
                include_docstrings=kw.get("include_docstrings", True),
            )
        ),
        # anchors module - takes source string, not path
        "anchors.find_anchors": lambda root, source=None, file_path=None, anchor=None, **kw: (
            find_anchors(
                source if source else _read_file(file_path),
                Anchor.parse(anchor) if isinstance(anchor, str) else anchor,
            )
        ),
        "anchors.resolve_anchor": lambda root, source=None, file_path=None, anchor=None, **kw: (
            resolve_anchor(
                source if source else _read_file(file_path),
                Anchor.parse(anchor) if isinstance(anchor, str) else anchor,
            )
        ),
        # cfg module - takes source string
        "cfg.build_cfg": lambda root, source=None, file_path=None, function_name=None, **kw: (
            build_cfg(source if source else _read_file(file_path), function_name)
        ),
        # dependencies module - takes source string
        "dependencies.extract_dependencies": lambda root, source=None, file_path=None, **kw: (
            extract_dependencies(source if source else _read_file(file_path))
        ),
        "dependencies.format_dependencies": lambda root, info=None, file_path=None, **kw: (
            format_dependencies(info) if info else format_dependencies(
                extract_dependencies(_read_file(file_path))
            )
        ),
        # complexity module
        ...
        "tree.render_tree": lambda root, node=None, **kw: (
            "\n".join(render_tree(node if node else build_tree(get_all_files(root))))
        ),
        "tree.build_tree": lambda root, files=None, **kw: (
            build_tree(files if files else get_all_files(root))
        ),
        # dwim module
        "dwim.list_tools": lambda root, **kw: list_tools(),
        "dwim.resolve_tool": lambda root, tool_name, **kw: resolve_tool(tool_name),
        "dwim.analyze_intent": lambda root, query, top_k=5, **kw: analyze_intent(query)[:top_k],
    }


def _format_tree(root: Path, depth: int = 2) -> str:
    ...
    def _walk(path: Path, prefix: str, level: int):
        ...
        for i, item in enumerate(items):
            ...
            if item.is_dir():
                ...
                _walk(item, prefix + extension, level + 1)
...
async def _execute_tool(name: str, arguments: dict[str, Any], tools: list) -> Any:
    ...
    # Find handler for this tool's api_path
    handler = dispatcher.get(tool.api_path)
    if handler is None:
        raise ValueError(f"Unknown API path: {tool.api_path}")
...
def _prompt_understand_file(file_path: Path) -> list[PromptMessage]:
    ...
    try:
        source = file_path.read_text()
        symbols = extract_python_skeleton(source)
        skeleton_text = format_skeleton(symbols)
...
def _prompt_prepare_refactor(file_path: Path, goal: str) -> list[PromptMessage]:
    ...
    try:
        ...
        source = file_path.read_text() if file_path.exists() else ""
        skeleton = extract_python_skeleton(source)
        formatted = format_skeleton(skeleton, include_docstrings=True)
