...
**server-less is our own project** (dogfooding). The proc macro lives at `/home/me/git/rhizone/server-less`. When the proc macro causes confusing behavior, investigate and note it — don't work around silently. Known quirk: the `#[cli]` proc macro treats **all** `&self` methods inside the annotated impl block as subcommands (unless the name starts with `_`). Helper methods like `display_*` must live in a **separate** `impl` block above the `#[cli]` block, not inside it. `display_with = "method_name"` references methods across impl blocks on the same struct.
