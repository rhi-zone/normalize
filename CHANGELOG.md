...
- **CA cache poisoning fix** — `normalize structure rebuild` no longer caches empty extraction results when a grammar's shared library is unavailable. Previously, running a rebuild without grammars loaded would write zero-symbol entries to `~/.config/normalize/ca-cache.sqlite` under the correct grammar key, causing subsequent rebuilds (even with grammars available) to return empty results from the cache. The fix: only write to the CA cache when `parser_for(grammar).is_some()` confirms the grammar was actually loaded. `EXTRACTOR_VERSION` bumped from "1" to "2" to automatically purge any already-poisoned entries on the next rebuild.
