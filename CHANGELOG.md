...
- **No `git` binary required** — all read-only git operations (commit history, file churn, diff, ref resolution, branch lookup, remote URL, tracked files, and budget metric comparisons) now use `gix` (pure-Rust gitoxide). A `git` binary in `$PATH` is no longer a runtime dependency for `normalize analyze`, `normalize rules run`, `normalize sessions`, `normalize ratchet`, or `normalize budget`. The last remaining git shell-outs in budget metrics (worktree checkout for symbol/complexity diffs and `git diff` for line/module/todo/dependency diffs) have been replaced with in-memory gix blob reads — no filesystem checkout needed. Write operations and a small number of operations without `gix` equivalents (`git blame`, `git status --porcelain`, path-filtered log count) remain as shell-outs.
