...
- **ANN search via sqlite-vec** — semantic search now uses a `vec_embeddings` virtual table (`vec0`) for approximate nearest-neighbor lookup instead of loading all vectors into memory. The extension is registered globally via `sqlite3_auto_extension` on first use; the top-50 ANN candidates are then re-ranked by the staleness penalty. Falls back to brute-force if the extension is unavailable (e.g. first run before schema migration).
