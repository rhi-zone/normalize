...
- **No `git` binary required** — all read-only git operations now use `gix` (pure-Rust gitoxide). This includes `git blame` (ownership, provenance, `view history`), `git status --porcelain` (stale-summary uncommitted change detection, `git.dirty`/`git.staged` rule source), path-filtered commit counts and last-commit lookup (`stale-summary`/`missing-summary` incremental cache), and `git rev-list --count` (coupling-clusters threshold). A `git` binary in `$PATH` is no longer a runtime dependency for any read-only normalize operation. Write operations (`git worktree add/remove`) remain as shell-outs (gix does not support these yet).
