...
- [x] Cross-cutting `--trend` and `--diff <ref>` modifiers on scoring commands — DONE for `complexity` and `length`. `complexity --diff HEAD~10` / `length --diff main` show per-function deltas sorted by |delta|. `complexity-trend` / `length-trend` show avg metric over N evenly-spaced historical commits. Shared `git_history.rs` (`run_in_worktree`, `resolve_ref`, `git_log_timestamps`, `select_snapshots`) extracted from `trend.rs`. `trend.rs` refactored to use it. `ScalarTrendReport` added for per-metric trends. Next: extend to `density` / `test_ratio` if desired (module-level, different shape).
