...
- [ ] **Idris `|||` doc comments** (low priority): `|||` is parsed as a plain `(comment)` in tree-sitter-idris — the grammar has no distinct `doc_comment` node. Two unblocking paths: (a) upstream grammar fix adding a dedicated node type; (b) add `#match?` predicate support to the decoration query runner so we can write `(comment) @decoration (#match? @decoration "^\\|\\|\\|")` to capture only leading-`|||` comments. Currently no predicates are evaluated anywhere (`QueryCursor::matches` returns all captures regardless). Until one of these lands, Idris decorations over-capture all comments (same as Go/C/Ruby). Pragmas (`%hint`, `%inline`) are each their own specific node — no generic `pragma` wrapper exists in the grammar.
