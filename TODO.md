...
- Remaining duplicate/clone detection improvements:
  ...
  - ~~**`duplicate-blocks` should elide literals by default**~~ — DONE (2026-03-10). `--scope blocks` now defaults to `elide_literals=true`; opt out with `--no-elide-literals`. `--elide-literals` flag still works for functions/similar mode. `similar-blocks` has no `--elide-literals` at all (add it — still TODO).
