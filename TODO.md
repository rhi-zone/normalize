...
- **Tier 2 surgical config invalidation (per-rule re-eval)** — when only one rule's behavior changed (newly-enabled, threshold/extra-field changed, or its `.scm` definition was edited), only that rule should re-run. Today the daemon falls back to a full reprime for any non-filter-only diff. Required hooks: route a single rule id through the syntax/fact/native engines, drop existing findings for that rule from per-engine blobs, merge new findings, rebuild the "all" blob and per-file rows. `ConfigDiff::rules_to_rerun` already names the affected rules. The `.scm` file diff (hashing each file under `.normalize/rules/` and comparing to a stored hash map) needs a small extension to the daemon's snapshot state — keep it next to `cached_rules` / `cached_walk` in `WatchedRoot`.
