...
The core should be a reusable Rust library (`crates/moss/`). Interfaces (CLI, HTTP, MCP, LSP) are thin wrappers around the library. The MCP server is autogenerated from the API via introspection.
