...
Spot-checking architecture (ooxml, dew, resin, burn-models) shows clean structure: domain-driven crate splits, trait-based APIs, good docs. At this scale, you're producing more code than you can personally review - but if tests are comprehensive, why does that matter? Tests *are* review. The real gap is workarounds that slip through: TODOs, placeholder implementations, "good enough for now" that becomes permanent. You can grep for `TODO`, search for common copout patterns - but heuristics aren't perfect. You'll catch most, not all. burn-models is known to not work correctly for many model types, not because of bad architecture, but because ML reimplementation is inherently hard (undocumented details, numeric precision, no ground truth to test against). Clean code ≠ correct code, and comprehensive tests only test what you thought to test.
