...
| `normalize-architecture` | Architectural metrics: coupling, cycles, layering, hubs | planned (inversion) `architecture` | Backs `architecture`/`depth-map`/`layering` (B3). Import-graph enabler landed (`normalize-index`); re-exports `build_import_graph`/`ImportGraph` from it. |
| `normalize-graph` | Pure graph algorithms: SCC, bridges, diamonds, chains, transitive edges | planned (inversion) `graph` | Verb carves out of `view` (`view graph`/`dependents`/`import-path`), not analyze/rank (B2). Refactored in place 2026-07-02. |
| `normalize-code-similarity` | Code similarity algorithms: MinHash LSH, normalized AST hashing, structural tokenization | planned (inversion) `similarity` | Backs `duplicates`/`duplicate-types`/`fragments` (B4). `clusters` is git-temporal, NOT here. |
