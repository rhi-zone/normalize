...
| crate | purpose | namespace (current / planned) | key notes |
|---|---|---|---|
| `normalize-architecture` | Architectural metrics: coupling, cycles, layering, hubs | planned (inversion) `architecture` | Backs `architecture`/`depth-map`/`layering` (B3). Gated on index/config enablers. |
