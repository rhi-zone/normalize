...
| `normalize-semantic` | Semantic retrieval layer: vector embeddings over structurally-derived chunks | `search` | Owns the top-level `search` verb (`SemanticCliService`, `cli` feature). Mounted at B7; `search`→`grep` alias dropped in the same change. |
