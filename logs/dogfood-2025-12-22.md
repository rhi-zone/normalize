...
**Command:** `normalize agent "Review the session_analysis.py file..."`

**Result:** FAILED - max turns reached

**Issues identified:**
1. **Path resolution failing**: Agent tried `session_analysis.py` without `src/normalize/` prefix
   - Intent: `skeleton session_analysis.py`
   - Output: `File not found: /home/me/git/normalize/session_analysis.py`
   - Expected: Should resolve to `src/normalize/session_analysis.py`
...
**Command:** `normalize workflow run vanilla --file src/normalize/session_analysis.py --arg "task=..."`
...
**Vanilla prompt format** (`src/normalize/prompts/vanilla.txt`):
```
tree.format [path]
...
**Key finding:** `normalize workflow run vanilla` != `normalize agent`
...
**The proper agentic loop is `normalize agent`** which uses DWIMLoop:
1. Get terse intent from LLM
2. Parse intent via DWIM
3. Execute tool via NormalizeAPI
4. Store result
5. Repeat until "done" or limit

**Recommendation:**
- Use `normalize agent "task"` for agentic execution with tools
...
3. Rust CLI has built-in fuzzy path resolution: `dwim.py` → `src/normalize/dwim.py`

**Test results:**
```
$ normalize agent "View the file dwim.py" --max-turns 3 --verbose
--- Turn 1 ---
Intent: view dwim.py
Output: { "path": "src/normalize/dwim.py", ... }  # Resolved correctly!
...
- Added fuzzy path example: `view dwim.py` resolves to `src/normalize/dwim.py`

**Test results:**
```
$ normalize agent "Show the resolve_core_primitive function in dwim.py"
--- Turn 1 ---
Intent: view dwim.py/resolve_core_primitive
Output: { "file": "src/normalize/dwim.py", "source": "def resolve_core_primitive..." }

--- Turn 2 ---
Intent: done resolve_core_primitive function in src/normalize/dwim.py located
Completed in 2 turns
```

**Working:**
- Fuzzy path resolution: `dwim.py` → `src/normalize/dwim.py`
...
- [x] Document that `normalize agent` is the proper agentic loop
